	...
	<view>
		...
		<view class="appointment">
			<view class="appointment-list" v-for="(item,index) in appointmentList" :key="index" @tap.stop="toDetails(item.id)">
				...
				<view class="appointment-btn-details" @tap.stop="toDetails(item.id)">查看详情</view>
			</view>
		</view>
		
		<view v-if="appointmentList.length == 0" class="text-center mt-100">暂无已完成工单</view>
	</view>
</template>
